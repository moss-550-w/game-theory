...
| M2 | 四象限主视图（静态） | QuadrantView 布局、四支柱锚定、主题色 | 看到四象限版图与四大支柱 | ⏭ 下一步 |
| M3 | 关系箭头 + 环绕层 | 继承/批判箭头、时间环、应用环（可开关） | 主视图叙事主干完整呈现 | 待开始 |
| M4 | 深度探索视图 | 象限点击 → 双重螺旋（历史线 / 数学线 + 交叉连线） | 单理论深度视图可进入与返回 | 待开始 |
| M5 | 透视模式 + 证明树 | 三种透视模式、多分支证明树（展开/收起） | 数学聚焦模式下浏览多证明路径 | 待开始 |
